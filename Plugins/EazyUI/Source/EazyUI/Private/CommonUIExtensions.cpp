...
void UCommonUIExtensions::PopContentFromLayer(UCommonActivatableWidget* ActivatableWidget, const FGameplayTag LayerName)
{
	...
	{
		UE_LOG(LogEazyUI, Warning, TEXT("[%s::%hs] -> ActivatableWidget is invalid."), *StaticClass()->GetName(), __func__);
		return;
	}
	...
	if (const ULocalPlayer* LocalPlayer = ActivatableWidget->GetOwningLocalPlayer())
	{
		if (const UEazyGameUIManagerSubsystem* UIManager = LocalPlayer->GetGameInstance()->GetSubsystem<UEazyGameUIManagerSubsystem>())
		{
			if (const UEazyGameUIPolicy* Policy = UIManager->GetCurrentUIPolicy())
			{
				if (const UEazyPrimaryGameLayout* RootLayout = Policy->GetRootLayout(LocalPlayer))
				{
