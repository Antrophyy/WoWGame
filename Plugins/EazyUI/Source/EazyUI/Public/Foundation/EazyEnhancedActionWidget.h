...
#include "EazyEnhancedActionWidget.generated.h"
...
// A widget that shows a platform-specific icon for the given input action. It can use either EI or Legacy input without needing EI settings to be enabled.
// If the legacy option is filled, it takes priority over EI.
UCLASS(BlueprintType, Blueprintable, ClassGroup=UI)
class EAZYUI_API UEazyEnhancedActionWidget : public UCommonActionWidget
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = EazyActionWidget)
