...
private:
	...
	void EnsureUniqueBinding(const UInputAction* Action, ETriggerEvent Event, UEnhancedInputComponent* EnhancedInputComponent);

	// Removes the EI binding for (Action, Event) if no more internal bindings reference it.
	void RemoveUniqueBinding(const UInputAction* Action, ETriggerEvent Event);
