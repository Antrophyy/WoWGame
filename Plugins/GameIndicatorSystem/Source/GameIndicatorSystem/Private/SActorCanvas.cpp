...
public:
	...
	FORCEINLINE void SetRotation(const float InRotation)
	{
...
EActiveTimerReturnType SActorCanvas::UpdateCanvas(double InCurrentTime, float InDeltaTime)
{
	...
	UGameIndicatorManagerComponent* IndicatorComponent = IndicatorComponentPtr.Get();
	if (IndicatorComponent == nullptr)
	{
		IndicatorComponent = UGameIndicatorManagerComponent::GetComponent(LocalPlayerContext.GetPlayerController());
		if (IndicatorComponent)
		{
			...
			for (UGameIndicatorDescriptor* Indicator : IndicatorComponent->Indicators)
			{
	...
	if (LocalPlayer)
	{
		...
		if (LocalPlayer->GetProjectionData(LocalPlayer->ViewportClient->Viewport, /*out*/ ProjectionData))
		{
			...
			for (int32 ChildIndex = 0; ChildIndex < CanvasChildren.Num(); ++ChildIndex)
			{
				...
				UGameIndicatorDescriptor* Indicator = CurChild.Indicator;
...
void SActorCanvas::SetComponentContainingSocketName(UGameIndicatorDescriptor* IndicatorDescriptor) const
{
	if (IndicatorDescriptor->ComponentSocketName == NAME_None || !IndicatorDescriptor->SceneComponent.IsValid())
	{
		return;
	}

	const TWeakObjectPtr<USceneComponent> Component = IndicatorDescriptor->SceneComponent;
	if (!Component->DoesSocketExist(IndicatorDescriptor->ComponentSocketName))
	{
		// Socket doesn't exist, try to find the socket on any other scene component.
		...
		{
			if (SceneComponent->DoesSocketExist(IndicatorDescriptor->ComponentSocketName))
			{
				IndicatorDescriptor->SceneComponent = SceneComponent;
				break;
			}
		}
	}
...
void SActorCanvas::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	...
	if (bShowAnyIndicators)
	{
		...
		for (int32 ChildIndex = 0; ChildIndex < SortedSlots.Num(); ++ChildIndex)
		{
			...
			NativePreIndicatorAddition(CurChild, AllottedGeometry, Indicator, OUT ScreenPosition);
...
int32 SActorCanvas::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, const int32 LayerId, const FWidgetStyle& InWidgetStyle, const bool bParentEnabled) const
{
...
void SActorCanvas::OnIndicatorAdded(UGameIndicatorDescriptor* Indicator)
{
...
void SActorCanvas::OnIndicatorRemoved(UGameIndicatorDescriptor* Indicator)
{
	RemoveIndicatorForEntry(Indicator);
...
void SActorCanvas::AddIndicatorForEntry(UGameIndicatorDescriptor* Indicator)
{
	// Async load the indicator, and pool the results so that it's easy to use and reuse the widgets.
	TSoftClassPtr<UGameBaseIndicatorWidget> IndicatorClass = Indicator->IndicatorWidgetClass;
	if (IndicatorClass.IsNull())
	{
		ensureAlwaysMsgf(false, TEXT("Indicator widget class is null for indicator %s. Please, select the correct type in the designer."), *Indicator->GetName());
		return;
	}

	SetComponentContainingSocketName(Indicator);
	
	TWeakObjectPtr<UGameIndicatorDescriptor> IndicatorPtr(Indicator);

	TWeakPtr<SActorCanvas> ActorCanvasWeakThis = SharedThis(this);
	FStreamableManager& StreamableManager = UAssetManager::Get().GetStreamableManager();
	TSharedPtr<FStreamableHandle> StreamingHandle = StreamableManager.RequestAsyncLoad(IndicatorClass.ToSoftObjectPath(), FStreamableDelegate::CreateLambda([IndicatorPtr, this, IndicatorClass, ActorCanvasWeakThis]
	 {
		 if (!ActorCanvasWeakThis.IsValid() || !IndicatorPtr.IsValid())
		 {
			 return;
		 }
		
	 	if (UGameIndicatorDescriptor* IndicatorDescriptor = IndicatorPtr.Get())
	 	{
			 // While async loading this indicator widget we could have removed it.
			 if (!AllIndicators.Contains(IndicatorDescriptor))
			 {
				 return;
			 }

			 // Create the widget from the pool.
			 if (UGameBaseIndicatorWidget* IndicatorWidget = IndicatorPool.GetOrCreateInstance(TSubclassOf<UGameBaseIndicatorWidget>(IndicatorClass.Get())))
			 {
				 IndicatorWidget->OnIndicatorRemovedAnimationFinished.AddSP(this, &SActorCanvas::IndicatorRemovedAnimationFinished);
				 IndicatorDescriptor->IndicatorWidget = IndicatorWidget;

				 IndicatorWidget->InitializeIndicatorWidget(IndicatorDescriptor);
				 IndicatorDescriptor->OnWidgetCreated.Broadcast(IndicatorWidget);

				 InactiveIndicators.Remove(IndicatorDescriptor);

				 AddActorSlot(IndicatorDescriptor)
				 [
					 SAssignNew(IndicatorDescriptor->CanvasHost, SBox)
					 [
...
void SActorCanvas::RemoveIndicatorForEntry(UGameIndicatorDescriptor* Indicator)
{
	if (UUserWidget* IndicatorWidget = Indicator->IndicatorWidget.Get())
	{
		bool bPlayRemoveAnimation = false;

		if (UGameBaseIndicatorWidget* BaseIndicatorWidget = Cast<UGameBaseIndicatorWidget>(IndicatorWidget))
		{
			// Play remove animation before deletion
			bPlayRemoveAnimation = true;
			BaseIndicatorWidget->NativeOnIndicatorPreDestruction();
			BaseIndicatorWidget->PlayIndicatorRemovedAnimation();
		}

		if (!bPlayRemoveAnimation)
		{
...
void SActorCanvas::RemoveCanvasSlot(UGameIndicatorDescriptor* Indicator)
{
...
void SActorCanvas::IndicatorRemovedAnimationFinished(UGameBaseIndicatorWidget* IndicatorWidget)
{
	UGameIndicatorDescriptor* IndicatorDescriptor = Cast<UGameIndicatorDescriptor>(IndicatorWidget->IndicatorDescriptor.Get());
	if (!IsValid(IndicatorWidget) || !IndicatorDescriptor)
	{
		return;
	}

	IndicatorWidget->StopAllAnimations();
	IndicatorWidget->NativeResetWidgetToDefault();
	IndicatorDescriptor->IndicatorWidget = nullptr;
	IndicatorPool.Release(IndicatorWidget);

	RemoveCanvasSlot(IndicatorDescriptor);

	IndicatorDescriptor->ManagerComponent->Indicators.Remove(IndicatorDescriptor);

	IndicatorWidget->OnIndicatorRemovedAnimationFinished.RemoveAll(this);
}

SActorCanvas::FScopedWidgetSlotArguments SActorCanvas::AddActorSlot(UGameIndicatorDescriptor* Indicator)
{
	...
	return FScopedWidgetSlotArguments{ MakeUnique<FSlot>(Indicator), this->CanvasChildren, INDEX_NONE
		...
		{
			if (const TSharedPtr<SActorCanvas> Canvas = WeakCanvas.Pin())
			{
...
void SActorCanvas::GetOffsetAndSize(const UGameIndicatorDescriptor* Indicator,
	FVector2D& OutSize, 
	FVector2D& OutOffset,
