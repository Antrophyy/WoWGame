#include "Foundation/RareBoundActionBar.h"
...
#define LOCTEXT_NAMESPACE "RareUI"

void URareBoundActionBar::SetDisplayOwningPlayerActionsOnly(bool bShouldOnlyDisplayOwningPlayerActions)
{
...
void URareBoundActionBar::OnWidgetRebuilt()
{
...
void URareBoundActionBar::SynchronizeProperties()
{
	Super::SynchronizeProperties();
}

void URareBoundActionBar::ReleaseSlateResources(const bool bReleaseChildren)
{
...
#if WITH_EDITOR
void URareBoundActionBar::ValidateCompiledDefaults(IWidgetCompilerLog& CompileLog) const
{
...
void URareBoundActionBar::HandleBoundActionsUpdated(const bool bFromOwningPlayer)
{
...
void URareBoundActionBar::HandleInputMappingsRebuiltUpdated()
{
	UpdateDisplay();
}

void URareBoundActionBar::UpdateDisplay()
{
	if (!bIsRefreshQueued)
	{
...
bool URareBoundActionBar::IsSafeToUpdateDisplay() const
{
	return !DoAnyActionButtonsHaveMouseCapture();
}

void URareBoundActionBar::HandleDeferredDisplayUpdate()
{
	...
	for (const ULocalPlayer* LocalPlayer : SortedPlayers)
	{
		if (LocalPlayer == OwningLocalPlayer || !bDisplayOwningPlayerActionsOnly)
		{
			...
			{
				if (const URareUIActionRouterBase* ActionRouter = ULocalPlayer::GetSubsystem<URareUIActionRouterBase>(LocalPlayer))
				{
					...
					TArray<FRareActionBindingHandle> FilteredBindings = ActionRouter->GetActiveBindings().FilterByPredicate([ActionRouter, PlayerInputType, &AcceptedBindings, this](const FRareActionBindingHandle& Handle) mutable
					{
						if (const TSharedPtr<FRareActionBinding> Binding = FRareActionBinding::FindBinding(Handle))
						{
					...
					Algo::Sort(FilteredBindings, [](const FRareActionBindingHandle& A, const FRareActionBindingHandle& B)
					{
						const TSharedPtr<FRareActionBinding> BindingA = FRareActionBinding::FindBinding(A);
						const TSharedPtr<FRareActionBinding> BindingB = FRareActionBinding::FindBinding(B);

						if (ensureMsgf(BindingA && BindingB, TEXT("The array filter above should enforce that there are no null bindings")))
						{
							auto GetNavBarPriority = [](const TSharedPtr<FRareActionBinding>& ActionBinding)
							{
					...
					for (FRareActionBindingHandle BindingHandle : FilteredBindings)
					{
						URareBoundActionButton* ActionButton = Cast<URareBoundActionButton>(CreateActionButton(BindingHandle));
...
UUserWidget* URareBoundActionBar::CreateActionButton(const FRareActionBindingHandle& BindingHandle)
{
	return CreateEntryInternal(ActionButtonClass);
}

void URareBoundActionBar::HandlePlayerAdded(const int32 PlayerIdx)
{
...
void URareBoundActionBar::HandlePlayerRemoved(const int32 PlayerIdx)
{
...
void URareBoundActionBar::HandledInputTypeUpdated(ECommonInputType)
{
	UpdateDisplay();
}

void URareBoundActionBar::MonitorPlayerActions(const ULocalPlayer* NewPlayer)
{
...
void URareBoundActionBar::ActionBarUpdateBegin()
{
	ActionBarUpdateBeginImpl();
}

void URareBoundActionBar::ActionBarUpdateEnd()
{
	ActionBarUpdateEndImpl();
}

bool URareBoundActionBar::DoAnyActionButtonsHaveMouseCapture() const
{
