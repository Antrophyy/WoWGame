...
EActiveTimerReturnType SActorCanvas::UpdateCanvas(double InCurrentTime, float InDeltaTime)
{
	...
	if (IndicatorComponent == nullptr)
	{
		...
		if (IndicatorComponent)
		{
			...
			for (UIndicatorDescriptor* Indicator : IndicatorComponent->Indicators)
			{
	...
	if (!IndicatorClass.IsNull())
	{
		...
		AsyncLoad(IndicatorClass, [this, IndicatorPtr, IndicatorClass]()
		{
			if (UIndicatorDescriptor* Indicator = IndicatorPtr.Get())
			{
				...
				if (UBaseIndicatorWidget* IndicatorWidget = IndicatorPool.GetOrCreateInstance(TSubclassOf<UBaseIndicatorWidget>(IndicatorClass.Get())))
				{
					...
					IndicatorWidget->InitializeIndicatorWidget(Indicator);
					Indicator->OnWidgetCreated.Broadcast(IndicatorWidget);
...
void SActorCanvas::RemoveIndicatorForEntry(UIndicatorDescriptor* Indicator)
{
	...
	if (UUserWidget* IndicatorWidget = Indicator->IndicatorWidget.Get())
	{
		...
		if (UBaseIndicatorWidget* BaseIndicatorWidget = Cast<UBaseIndicatorWidget>(IndicatorWidget))
		{
			// Play remove animation before deletion
			bPlayRemoveAnimation = true;
			BaseIndicatorWidget->NativeOnIndicatorPreDestruction();
			BaseIndicatorWidget->PlayIndicatorRemovedAnimation([BaseIndicatorWidget, Indicator, this, IndicatorWidget]
			{
				if (!IsValid(Indicator->IndicatorWidget.Get()) || !IsValid(Indicator))
					return;

				BaseIndicatorWidget->StopAllAnimations();
				BaseIndicatorWidget->NativeResetWidgetToDefault();
...
void SActorCanvas::RemoveCanvasSlot(UIndicatorDescriptor* Indicator)
{
